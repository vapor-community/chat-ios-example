...
class ChatMessageCell : UITableViewCell {
    ...
    struct Appearance {
        static var opponentColor = UIColor.clearColor() //UIColor(red: 0.142954, green: 0.60323, blue: 0.862548, alpha: 0.88)
        static var userColor = UIColor.clearColor() //UIColor(red: 0.14726, green: 0.838161, blue: 0.533935, alpha: 1)
    ...
    private func styleTextViewForSentBy(sentBy: ChatMessage.SentBy) {
...
protocol ChatControllerDelegate: class {
    func chatController(chatController: ChatController, didAddNewMessage message: ChatMessage)
}
...
class ChatController : UIViewController, UITableViewDelegate, UITableViewDataSource, ChatInputDelegate {
    ...
    private let chatInput = ChatInput(frame: CGRectZero)
    ...
    private func setupTableView() {
        ...
        tableView.registerClass(ChatMessageCell.classForCoder(), forCellReuseIdentifier: "identifier")
    ...
    func chatInputDidResize(chatInput: ChatInput) {
        self.scrollToBottom()
    }

    func chatInput(chatInput: ChatInput, didSendMessage message: String) {
...
class ChatInput : UIView, StretchyTextViewDelegate {
    ...
    private let textView = StretchyTextView(frame: CGRectZero, textContainer: nil)
    ...
    func setupSendButton() {
        ...
        self.sendButton.addTarget(self, action: #selector(ChatInput.sendButtonPressed(_:)), forControlEvents: .TouchUpInside)
    ...
    func stylize() {
        ...
        self.blurredBackgroundView.hidden = true
        self.backgroundColor = UIColor(white: 0.0, alpha: 0.25)
    }

    // MARK: StretchyTextViewDelegate

    func stretchyTextViewDidChangeSize(textView: StretchyTextView) {
    ...
    func stretchyTextView(textView: StretchyTextView, validityDidChange isValid: Bool) {
        self.sendButton.enabled = isValid
    }

    func stretchyTextViewDidReturn(textView: StretchyTextView) {
